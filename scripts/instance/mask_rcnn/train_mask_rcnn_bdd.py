...
class DrivableMapsAccMetric(mx.metric.EvalMetric):
    def __init__(self):
        super(DrivableMapsAccMetric, self).__init__('MaskAcc')
    ...
    def update(self, labels, preds):
        ...
        maps_mask = preds[0]
        # rcnn_mask (b, c, h, w)
        pred_label = mx.nd.sigmoid(maps_mask) >= 0.5
        label = labels[0] >= 0.5
        # label (b, c, h, w)
        num_acc = mx.nd.sum((pred_label == label))

        self.sum_metric += num_acc.asscalar()

def get_dataset(dataset, args):
    if dataset.lower() == 'coco':
        train_dataset = gdata.BDDInstance(root='/Volumes/DATASET/BDD100k/bdd100k/', splits='bdd100k_to_coco_labels_images_val2018', skip_empty=True, use_color_maps=False)
        val_dataset = gdata.BDDInstance(root='/Volumes/DATASET/BDD100k/bdd100k/', splits='bdd100k_to_coco_labels_images_val2018', skip_empty=False, use_color_maps=False)
...
def get_dataloader(net, train_dataset, val_dataset, batch_size, num_workers):
    ...
    train_loader = mx.gluon.data.DataLoader(
        train_dataset.transform(BDDMaskRCNNDefaultTrainTransform(net.short, net.max_size, net)),
        ...
        for i, batch in enumerate(train_data):
            ...
            with autograd.record():
                for data, label, gt_drivable_maps, rpn_cls_targets, rpn_box_targets, rpn_box_masks in zip(*batch):
                    ...
                    drivable_maps_loss = drivable_maps_loss(drivable_maps_pred, gt_drivable_maps) 
                    # overall losses
                    ...
                    add_losses[4].append([[gt_drivable_maps], [drivable_maps_pred]])
...
if __name__ == '__main__':
    ...
    # net = get_model(net_name, pretrained=True, root='/Volumes/DATASET/mxnet')
