...
def parse_args():
    ...
    parser.add_argument('--start-epoch', type=int, default=17,
    ...
    parser.add_argument('--save-niter', type=int, default=1000,
                        help='Saving parameters epoch interval, best model will always be saved.')
    ...
    parser.add_argument('--one-hot-label', action='store_true',
                        help='label one-hot.')
    args = parser.parse_args()
    args.epochs = int(args.epochs) if args.epochs else 28
...
class DrivableMapsAccMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        num_inst = mx.nd.array([1])
        pixel_labeled = mx.nd.sum(label > 0)
        pixel_correct = mx.nd.sum((pred == label)*label)
        # print('pixel_correct', pixel_correct)
        pixel_accuracy = 1.0 * pixel_correct / (pixel_labeled + 0.000001)
        self.sum_metric += pixel_accuracy.asscalar()
        self.num_inst += num_inst.asscalar()

class DrivableMapsSoftmaxAccMetric(mx.metric.EvalMetric):
    def __init__(self, one_hot_label=False):
        super(DrivableMapsSoftmaxAccMetric, self).__init__('DrivableMapsSoftmaxAcc')
    ...
    def update(self, labels, preds):
        ...
        label, weight = labels
        pred = mx.nd.softmax(preds[0], axis=1) >0.5

        # calculate num_inst
        # print('pred',preds[0])
        # print('pred2',pred)
        # print('pred3',mx.nd.sum(pred,axis=1))
        # print('pred4',pred>0.5)
        # print('label', label)
        num_inst = mx.nd.array([1])
        pixel_labeled = mx.nd.sum(label > 0)
        # print('pixel_labeled', pixel_labeled)
...
def get_dataset(dataset, args):
    if dataset.lower() == 'bdd':
        train_dataset = gdata.BDDInstance(root='/data1/datasets/bdd100k/', splits='bdd100k_to_coco_labels_images_train2018', skip_empty=True, use_color_maps=False)
        val_dataset = gdata.BDDInstance(root='/data1/datasets/bdd100k/', splits='bdd100k_to_coco_labels_images_val2018', skip_empty=False, use_color_maps=False)
        val_metric = BDDInstanceMetric(val_dataset, args.save_prefix + '_eval', cleanup=True)
    else:
        raise NotImplementedError('Dataset: {} not implemented.'.format(dataset))
    return train_dataset, val_dataset, val_metric

def get_dataloader(net, train_dataset, val_dataset, batch_size, num_workers, one_hot_label):
    """Get dataloader."""
    train_bfn = batchify.Tuple(*[batchify.Append() for _ in range(6)])
    print('get_dataloader', net.max_size)
    train_loader = mx.gluon.data.DataLoader(
        train_dataset.transform(BDDMaskRCNNDefaultTrainTransform(net.short, net.max_size, net, segm_scale = 1/4.,one_hot_label=one_hot_label)),
...
def save_niter_params(net, logger,epoch, niter, prefix):
    logger.info('[Epoch {}] Saving parameters to {}'.format(
        epoch, '{:s}_{:04d}_{:04d}.params'.format(prefix, epoch, niter)))
    net.save_parameters('{:s}_{:04d}.params'.format(prefix, epoch))

def save_epoch_params(net, logger,epoch, niter, prefix):
    logger.info('[Epoch {}] Saving parameters to {}'.format(
        epoch, '{:s}_{:04d}_{:s}.params'.format(prefix, epoch, niter)))
    net.save_parameters('{:s}_{:04d}.params'.format(prefix, epoch))
...
def validate(net, val_data, ctx, eval_metric):
    ...
    for ib, batch in enumerate(val_data):
        ...
        for x, im_info in zip(*batch):
            ...
            segm_drivable_maps.append(drivable_maps)
        # update metric
        for det_bbox, det_id, det_score, segm_maps, det_info in zip(det_bboxes, det_ids, det_scores, segm_drivable_maps, det_infos):
            # det_bbox_metric = []
            # det_ids_metric = []
            # det_scores_metric = []
            # segm_maps_metric = []
            # print('det_info.shape[0]', det_info.shape[0]) #1
            for i in range(det_info.shape[0]):
                ...
                # print( len(det_bbox),  len(det_id),  len(det_score), len(segm_maps)) 103 103 103 1
                eval_metric.update(det_bbox, det_id, det_score, mx.nd.zeros_like(segm_maps))
            # segm_maps = segm_maps.asnumpy()
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    rcnn_drivable_maps_loss = mx.gluon.loss.SigmoidBinaryCrossEntropyLoss(from_sigmoid=False)
    metrics = [mx.metric.Loss('RPN_Conf'),
               mx.metric.Loss('RPN_SmoothL1'),
               mx.metric.Loss('RCNN_CrossEntropy'),
               mx.metric.Loss('RCNN_SmoothL1'),
               mx.metric.Loss('RCNN_Drivable_maps')]
    ...
    rcnn_acc_map_metric = DrivableMapsAccMetric()
    if args.one_hot_label:
        rcnn_acc_map_metric =DrivableMapsSoftmaxAccMetric()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        logger.info("[Epoch {}] start from [iter {}]".format(epoch, args.start_iter))
    
        for i, batch in enumerate(train_data, start=start_iter):
            ...
            with autograd.record():
                for data, label, gt_drivable_maps, rpn_cls_targets, rpn_box_targets, rpn_box_masks in zip(*batch):
                    # print( mx.nd.sum(data >= 1))
                    # print('0.5', mx.nd.sum(gt_drivable_maps >= 0.5))
                    # print('0.5', mx.nd.sum(gt_drivable_maps == 0.5))
                    # print('1', mx.nd.sum(gt_drivable_maps == 1))
                    # print(batch_size)
                    gt_label = label[:, :, 4:5]
                    gt_box = label[:, :, :4]
                    cls_pred, box_pred, drivable_maps_pred, roi, samples, matches, rpn_score, rpn_box, anchors = net(data, gt_box)
                    # losses of rpn
                    ...
                    # loss of drivable maps
                    # resize = Resize(size=preds.shape)
                    # label = resize(labels[0].transpose((1,2,0))).transpose(2,0,1)
                    # print('drivable_maps_pred',drivable_maps_pred.shape, gt_drivable_maps.shape)
                    maps_weight = 1.
                    # if args.one_hot_label:
                    #     drivable_maps_pred_prob = mx.nd.softmax(drivable_maps_pred, axis=1)
                    # else:
                    #     drivable_maps_pred_prob = drivable_maps_pred
                    drivable_maps_loss = rcnn_drivable_maps_loss(drivable_maps_pred, gt_drivable_maps) # * maps_weight
                    # overall losses
                
                    losses.append(rpn_loss.sum() + rcnn_loss.sum() + drivable_maps_loss.sum())
                    ...
                    metric_losses[4].append(drivable_maps_loss.sum())
                    ...
                    add_losses[4].append([[gt_drivable_maps, drivable_maps_pred>=0.5], [drivable_maps_pred]])
                    # add_losses[4].append([[drivable_maps_pred_prob, gt_drivable_maps>=0.5], [drivable_maps_pred]])
            ...
            if args.log_interval and not (i + 1) % args.log_interval:
                ...
                btic = time.time()
            # print('rcnn_acc_map_metric', rcnn_acc_map_metric.get())
            if not (i+1) % args.save_niter:
                save_niter_params(net, logger, epoch, i,  args.save_prefix)
        start_iter = 0
        save_epoch_params(net, logger, epoch, 'finished',  args.save_prefix)
...
if __name__ == '__main__':
    ...
    # net = get_model(net_name, pretrained=True, root='/home/user/.mxnet/models')
    ...
    if args.resume.strip():
        # '/home/user/.mxnet/models/mask_rcnn_resnet50_v1b_bdd.params'
    ...
    train_data, val_data = get_dataloader(
        net, train_dataset, val_dataset, args.batch_size, args.num_workers, args.one_hot_label)
